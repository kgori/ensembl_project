...
class SpeciesTree(ete2.Tree):
    ...
    def _add_duplication_nodes(self, gene_tree_node, already_deleted = [], **kwargs):                                                                   
        # CASE 1 - duplication                                                          # Record gain at mrca, then recurse on child nodes
        ...
        # CASE 2 - root node                                                            # Just recurse on child nodes
        elif not gene_tree_node.up:
            print 'ROOT NODE FOUND, RECURSING'
            for child in gene_tree_node.children:
                self._add_duplication_nodes(child, already_deleted=already_deleted)
        # CASE 3 - speciation following duplication                                     # Using mrca found at parent duplication node,
        elif gene_tree_node.type()!='D' and gene_tree_node.up.type()=='D':              # record deletions under this node in already_
            print 'SPECIATION FOLLOWING DUPLICATION',already_deleted                    #-deleted list and _already_deleted global,  
            subtree = self._find_mrca(gene_tree_node.up._taxon_set)
            # subtree = gene_tree_node.up._species_subtree                   # then recurse on child nodes
        ...
        # CASE 4 - leaf node                                                            # base case, end recursion
